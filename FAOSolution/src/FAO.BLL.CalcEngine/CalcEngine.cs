...
using FAO.BLL.CalcEngine.Conventions;
...
namespace FAO.BLL.CalcEngine
{
    public class CalcEngine : IBACalcEngine
    {
        ...
        public CalcEngine()
        {
            ...
            //m_pObjAdjAlloc = new BAFASAdjustmentAllocator();
        }
        ...
        public bool CalculateMonthlyProjection(IBADeprScheduleItem pObjSch, ref IBAPeriodDeprItem pdItem, DateTime startDate, DateTime endDate, out List<IBAPeriodDetailDeprInfo> pOutList)
        {
            ...
            if (CalendarManager.CycleList.Count == 0)
            {
            ...
            SetAutoTruckLimit(pObjSch.AutoTruckLimit);

            // calt
            IBAPeriodDeprItem tpdItem = null;
            if (pObjSch.DeprMethod == "RV" && m_pdItem.StartDate > DateTime.MinValue && m_pdItem.StartDate == m_pdItem.EndDate)
            {
                tpdItem = new BAFASPeriodDeprItem();
                tpdItem = m_pdItem;
                m_IsRVMethod = true;
            }
            ...
            if (pObjSch.AcquisitionValue < -0.005)
            {
                bFlipSign = true;
                helper.FlipSign(ref pObjSch, ref tpdItem);
            }

            if (!CalculatePDItemList(pObjSch, out objPDItems, dtCalcEndDate, ref tpdItem, false))
                return false;

            if (bFlipSign)
            {
                helper.FlipSign(ref objPDItems);
            }

            if (pObjSch.Bonus911Percent == 100)
                fixLastItem(pCalendar, ref objPDItems);

            setPeriodDetailDeprInfo(pObjSch, pCalendar, startDate, endDate, m_bIsLimitUsed, objPDItems, out pOutList);
        ...
        public bool CalculateProjection(IBADeprScheduleItem pObjSch, ref IBAPeriodDeprItem pdItem, out List<IBAPeriodDetailDeprInfo> pOutList)
        {
            IBACalendar pCalendar;
            IBAFiscalYear pRunDateYear;
            IBACalendarManager CalendarManager = null;
            ...
            //pdItem.AdjustAmount = 510M;
            //pdItem.BeginYearAccum = 1290;
            //pdItem.StartDate = new DateTime(2013, 7, 31);
            //pdItem.EndDate = new DateTime(2013, 7, 31);
        ...
        public IBAPeriodDetailDeprInfo CalculateDepreciation(IBADeprScheduleItem pObjSch, ref IBAPeriodDeprItem pdItem, DateTime dtRunDate)
        {
            List<IBAPeriodDetailDeprInfo> pTmpList;
            IBAPeriodDetailDeprInfo periodItem = new PeriodDetailDeprInfo();
            IBAPeriodDetailDeprInfo tmpPdi = new PeriodDetailDeprInfo();

            //CalculateProjection(pObjSch, out pTmpList);
            CalculateMonthlyProjection(pObjSch, ref pdItem, DateTime.MinValue, DateTime.MaxValue, out pTmpList);
            ...
            IBACalendar pCalendar = pObjSch.Calendar;
            IBAFiscalYear pRunDateYear;
            IBACalcPeriod pRunDatePeriod;
            pCalendar.GetFiscalYear(dtRunDate, out pRunDateYear);
            pCalendar.GetPeriod(dtRunDate, out pRunDatePeriod);

            periodItem.FiscalYearStartDate = pRunDateYear.YRStartDate;
            periodItem.FiscalYearEndDate = pRunDateYear.YREndDate;
            periodItem.PeriodStartDate = pRunDatePeriod.PeriodStart;
            periodItem.PeriodEndDate = pRunDatePeriod.PeriodEnd;
            if (dtRunDate > pObjSch.PlacedInServiceDate && dtRunDate < m_deemedStartDate)
                periodItem.FiscalYearBeginAccum = 0;
            else if (dtRunDate > m_deemedStartDate)
                periodItem.FiscalYearBeginAccum = tmpPdi.FiscalYearEndAccum;

            return periodItem;
        }

        public IBAPeriodDetailDeprInfo CalculateDisposal(IBADeprScheduleItem pObjSch, ref IBAPeriodDeprItem pdItem)
        {
            List<IBAPeriodDetailDeprInfo> pTmpList;
            IBAPeriodDetailDeprInfo periodItem = new PeriodDetailDeprInfo();
            IBAPeriodDetailDeprInfo tmpPdi = new PeriodDetailDeprInfo();
            ...
            foreach (IBAPeriodDetailDeprInfo pdi in pTmpList)
            {
                if (pdi.PeriodStartDate <= pObjSch.DispDate && pObjSch.DispDate <= pdi.PeriodEndDate || pdi.PeriodEndDate >= new DateTime(3000, 1, 1))
                {
                    periodItem = pdi;
                    return periodItem;
                }
            ...
            IBAFiscalYear pRunDateYear;
            IBACalcPeriod pRunDatePeriod;
            pCalendar.GetFiscalYear(pObjSch.DispDate, out pRunDateYear);
            pCalendar.GetPeriod(pObjSch.DispDate, out pRunDatePeriod);

            periodItem.FiscalYearStartDate = pRunDateYear.YRStartDate;
            periodItem.FiscalYearEndDate = pRunDateYear.YREndDate;
            periodItem.PeriodStartDate = pRunDatePeriod.PeriodStart;
            periodItem.PeriodEndDate = pRunDatePeriod.PeriodEnd;
            if (pObjSch.DispDate > pObjSch.PlacedInServiceDate)
                periodItem.FiscalYearBeginAccum = tmpPdi.FiscalYearEndAccum;
        ...
        public bool CalculatePDItemList(IBADeprScheduleItem pObjSch, out List<IBAPeriodDeprItem> pColPDIList, DateTime dtCalcEndDate, ref IBAPeriodDeprItem pObjPDItem, bool bAdjFlag)
        {
            ...
            if (!(CreateDeprMethod(deprMethod, bAdjFlag, out pObjAutoDeprMethod)) ||
                ...
                return false;
            IBADeprMethod pObjTmpAutoDeprMethod = pObjAutoDeprMethod;
            ...
            //   RDBJ 3/12/2000
            {
                IBASwitchDepr swDepr = (pObjDeprMethod) as IBASwitchDepr;
                IBADeprMethod tmpMeth;

                if (swDepr != null)
                {
                    m_sDBToSL = string.Empty;
                    swDepr.GetCurrentMethod(m_sCalcFlags, out tmpMeth);
                    m_sDBToSL = tmpMeth.ParentFlags;
                    pObjDeprMethod = null;
            ...
            if (bonus911Percent > 0 && m_dLimitArray[0] > 0)
            {
                ...
                // every year we need to update the PIS date limit based on congress (currently it is 12/31/2014)
                else if (new DateTime(2008, 1, 1) <= dtPISDate && dtPISDate <= new DateTime(2014, 12, 31))
                {
            ...
            if (m_pObjAdjAlloc != null)
            {
                ...
                m_pObjAdjAlloc.BeginInformation = pObjPDItem;
            }
                    ...
                    || bAdjStillNeeded))
            {
                    ...
                    d168Pct > 0 && !bPostLife)
                {
                    ...
                    // apply bus use
                    pObjAutoDeprMethod.PostUsageDeduction = pObjSch.Bonus911Amount;
                    ...
                    if (!pObjFYCalcer.IsFirstYear())
                    {
                        ...
                        pObjAutoDeprMethod = pObjTmpAutoDeprMethod;
                        pObjFYCalcer.DeprMethod = (pObjAutoDeprMethod);
                    }
...

                {
                        ...
                        if (m_pObjAdjAlloc != null)	// KENT START fix MS MC-00039
                        {
                            if (dtAdjCalcStart == dtCalcStartDate)
                            {
                                ...
                                if (!(hr = m_pObjAdjAlloc.CalculateAdjustment(dtStAdjCalcStart, ref dStAdjustmentAmount, ref sStAdjFlag, ref dtStAdjPeriodStart)))
                                    return hr;
                                if (sStAdjFlag != string.Empty && sStAdjFlag[0] != 0)
                                {
                                    double dTemp;
                    ...
                    if (m_pObjAdjAlloc != null)
                    {
                        ...
                        if (m_sAdjFlag != string.Empty && m_sAdjFlag[0] != 0)
                            bAdjStillNeeded = false;
                    }
                    else
                    {
                    ...
                    else
                    {
                        ...
                        //
                        if (dAdjustmentAmount != 0.0 && (m_sAdjFlag) != string.Empty && (m_sAdjFlag)[0] == 'a' && dtAdjPeriodStart == dtAdjCalcStart)
                        {
            ...
            if (!atLeastOneItemBuilt)
            {
                ...
                if (dtCalcStartDate > DateTime.MinValue)
                {
                    ...
                    if (dtCalcStartDate <= YearEnd && dtCalcStartDate >= YearStart)		//KENT
                    {
                        ...
                        DateTime tmpDate = dtCalcStartDate;
                        if (Math.Abs(dPriorAccum) < 0.005 && Math.Abs(dBeginPeriodAccum) < 0.005)
                            tmpDate = YearStart;

                        pPDItem.StartDate = (tmpDate);
                        pPDItem.EndDate = (YearEnd);
                        iFYNum = pObjIFY.FYNum;
                        pPDItem.FYNum = (iFYNum);
                        pObjIFY.GetPeriodWeights(tmpDate, YearEnd, out iPdWeights);
                ...
                //
                {
                    ...
                    //
                    {
                        ...
                        if (m_pObjAdjAlloc != null)
                        {
                            ...
                            if ((m_sAdjFlag) != string.Empty && m_sAdjFlag[0] == 'a' && dAdjustmentAmount > 0)
                                if (m_sAdjFlag != string.Empty && m_sAdjFlag.Contains("a"))
                                    m_sCalcFlags += "a";
                        }
                        else
                        {
        ...
        public bool SmartDeprMethodCreator(string progId, out IBADeprMethod pVal)
        {
            ...
            else if (progId.CompareTo("SageFASCalcEngine.MACRSTable") == 0)
            {
                pVal = new MACRSTable();
                return true;
            }
            ...
            else if (progId.CompareTo("SageFASCalcEngine.ACRSTable") == 0)
            {
                pVal = new ACRSTable();
                return true;
            }
            else if (progId.CompareTo("SageFASCalcEngine.AltACRSTable") == 0)
            {
                pVal = new AltACRSTable();
                return true;
            }
            ...
            else if (progId.CompareTo("SageFASCalcEngine.AltMACRSTable") == 0)
            {
                pVal = new AltMACRSTable();
                return true;
            }
            ...
            else if (progId.CompareTo("SageFASCalcEngine.CustomDeprMethod") == 0)
            {
                pVal = new CustomDeprMethod();
                return true;
            }
            pVal = null;
            return false;
        ...
        {
            BkTypeEnum type = schedule.BookType;
        ...
        private void setPeriodDetailDeprInfo(List<IBAPeriodDeprItem> pInList, out List<IBAPeriodDetailDeprInfo> pOutList)
        {
            ...
            foreach (IBAPeriodDeprItem pdi in pInList)
            {
                ...
                obj.PeriodBeginAccum = pdi.BeginYTDExpense;
                obj.PeriodDeprAmount = pdi.DeprAmount;
                obj.PeriodEndAccum = (decimal)(pdi.BeginYTDExpense + pdi.DeprAmount);
                obj.CalcFlags = pdi.CalcFlags;

                if (pdi.EndDate >= new DateTime(2999, 12, 31))
                    return;
        ...
        private void fixLastItem(IBACalendar pCalendar, ref List<IBAPeriodDeprItem> pList)
        {
            IBAFiscalYear objFY;
            IBAPeriodDeprItem pdi = pList[pList.Count - 1];
            pCalendar.GetFiscalYear(pdi.StartDate, out objFY);
            pdi.EndDate = objFY.YREndDate;
            IBAPeriodDeprItem newpdi = new PeriodDeprItem();
            pdi.Clone(out newpdi);
            newpdi.StartDate = objFY.YREndDate.AddDays(1);
            newpdi.EndDate = new DateTime(3000, 12, 31);
            pList.Add(newpdi);
        }

        private void setPeriodDetailDeprInfo(IBADeprScheduleItem pObjSch, IBACalendar pCalendar, DateTime startDate, DateTime endDate, bool isAuto, List<IBAPeriodDeprItem> pInList, out List<IBAPeriodDetailDeprInfo> pOutList)
        {
            IBAPeriodDetailDeprInfo obj;
            PeriodDetailDeprInfo objInfo;
            IBAFiscalYear objFY;
            ...
            if (m_pdItem.StartDate != DateTime.MinValue && m_pdItem.StartDate == m_pdItem.EndDate)
                startDate = m_pdItem.StartDate;
            ...
            if (endDate == null || !DateTimeHelper.IsValid(endDate))
            ...
            foreach (IBAPeriodDeprItem pdi in pInList)
            {
                if (pdi.StartDate > DateTime.MinValue)
                {
                    ...
                    if (objFY.YRStartDate < m_deemedEndDate && m_deemedEndDate <= objFY.YREndDate && !isAuto)
                    {
                        dtEndDate = m_deemedEndDate;
                        if (dtEndDate > pdi.EndDate)
                            dtEndDate = pdi.EndDate;
                    }
                    else
                    {
                    ...
                    if (dtEndDate >= new DateTime(2999, 12, 31))
                        break;
                    ...
                    iPeriodNum = 0;
                    decimal prevAccume = pdi.BeginYTDExpense;
                    double prevEndAccume = 0;
                    do
                    {
                        objInfo = new PeriodDetailDeprInfo();
                        obj = (IBAPeriodDetailDeprInfo)objInfo;
                        ...
                        if (iWeight > 0 && iPeriodNum < iWeight)
                            monthlyAmount = (double)(pdi.DeprAmount / iWeight);
                        else
                            monthlyAmount = 0;
                        ...
                        if (iPeriodNum == 0)
                        {
                            obj.PeriodDeprAmount = (decimal)Currency.FormatCurrency(monthlyAmount);
                            obj.PeriodBeginAccum = (decimal)Currency.FormatCurrency((double)(pdi.BeginYTDExpense));
                        }
                        else
                        {
                            //obj.PeriodBeginAccum = iPeriodNum * monthlyAmount;
                            //obj.PeriodBeginAccum = prevAccume + (decimal)monthlyAmount;
                            if (Math.Abs(monthlyAmount) > 0.0001)
                                obj.PeriodDeprAmount = (decimal)Currency.FormatCurrency((iPeriodNum + 1) * monthlyAmount - ((double)prevEndAccume - Currency.FormatCurrency((double)(pdi.BeginYTDExpense))));
                            obj.PeriodBeginAccum = (decimal)prevEndAccume;
                        }
                        prevAccume = obj.PeriodBeginAccum;
                        //obj.PeriodDeprAmount = (decimal)monthlyAmount;
                        obj.PeriodEndAccum = obj.PeriodBeginAccum + obj.PeriodDeprAmount;
                        prevEndAccume = (double)obj.PeriodEndAccum;
                        obj.CalcFlags = pdi.CalcFlags;

                        DateTime preDate = DateTime.MinValue;
                        if (pOutList.Count > 0)
                            preDate = pOutList[pOutList.Count - 1].PeriodStartDate;
                        if (obj.PeriodEndDate >= startDate && obj.PeriodStartDate <= endDate && obj.PeriodStartDate != preDate)
                            pOutList.Add(objInfo);
            ...
            {
                if (m_IsRVMethod)
                    setPeriodDetailDeprInfoForBeginInfoRV(pObjSch, pCalendar, ref pOutList);
                else
                    setPeriodDetailDeprInfoForBeginInfo(pObjSch, pCalendar, ref pOutList);
                //get the begin date row
                if (pOutList.Count > 0)
                {
                    var beginDtItem = pOutList[0];
                    IBAFiscalYear beginDtFY;
                    IBAFiscalYear lastYearBeforeBeginDtFY;
                    pCalendar.GetFiscalYear(beginDtItem.FiscalYearStartDate, out beginDtFY);
                    if (beginDtFY != null && (m_pdItem.BeginYearAccum != 0 || m_pdItem.BeginPeriodAccum != 0))
                    {
                        pCalendar.GetFiscalYear(beginDtFY.YREndDate.AddYears(-1), out lastYearBeforeBeginDtFY);
                        IBACalcPeriod lastPeriodBeforeBeginDt;
                        lastYearBeforeBeginDtFY.GetPeriod(lastYearBeforeBeginDtFY.YREndDate, out lastPeriodBeforeBeginDt);
                        PeriodDetailDeprInfo beginAccumItem = new PeriodDetailDeprInfo()
                        {
                            PeriodDeprAmount = m_pdItem.BeginYearAccum,
                            PeriodEndAccum = m_pdItem.BeginYearAccum,
                            FiscalYearEndAccum = m_pdItem.BeginYearAccum,
                            PeriodStartDate = lastPeriodBeforeBeginDt.PeriodStart,
                            PeriodEndDate = lastPeriodBeforeBeginDt.PeriodEnd,
                            FiscalYearStartDate = lastYearBeforeBeginDtFY.YRStartDate,
                            FiscalYearEndDate = lastYearBeforeBeginDtFY.YREndDate
                        };
                        pOutList.Insert(0, beginAccumItem);
                    }
                }
                //if (pOutList.Count > 0)
                //    pOutList.RemoveAt(0);
        ...
        private void setPeriodDetailDeprInfoForBeginInfo(IBADeprScheduleItem pObjSch, IBACalendar pCalendar, ref List<IBAPeriodDetailDeprInfo> pInList)
        {
            IBAFiscalYear objFY;
            IBACalcPeriod objPeriod;
            DateTime beginPeriodStart;
            DateTime beginPeriodEnd;
            decimal adjustmentAmt = 0;
            decimal prioAccum = 0;

            decimal dBeginYearAccum = m_pdItem.BeginYearAccum;

            pCalendar.GetFiscalYear(m_pdItem.StartDate, out objFY);
            objFY.GetPeriod(m_pdItem.StartDate, out objPeriod);
            beginPeriodStart = objPeriod.PeriodStart;
            beginPeriodEnd = objPeriod.PeriodEnd;

            foreach (IBAPeriodDetailDeprInfo pdi in pInList)
            {
                if (objFY.YRStartDate <= pdi.PeriodStartDate && pdi.PeriodEndDate <= objFY.YREndDate)
                {
                    if (beginPeriodStart >= pdi.PeriodStartDate && pdi.PeriodEndDate <= beginPeriodEnd)
                    {
                        adjustmentAmt = pdi.FiscalYearBeginAccum + pdi.PeriodEndAccum - (decimal)(m_pdItem.BeginPeriodAccum);
                        pdi.FiscalYearBeginAccum = dBeginYearAccum;
                        pdi.FiscalYearDeprAmount = pdi.FiscalYearEndAccum - pdi.FiscalYearBeginAccum;
                        pdi.PeriodBeginAccum = 0;
                        pdi.PeriodDeprAmount = m_pdItem.BeginYTDExpense;
                        pdi.PeriodEndAccum = pdi.PeriodBeginAccum + pdi.PeriodDeprAmount + pdi.AdjustmentAmt;
                        prioAccum = pdi.PeriodEndAccum;
                        if (objFY.YREndDate.ToUniversalTime().ToString("d") == m_pdItem.StartDate.ToUniversalTime().ToString("d"))
                        {
                            objFY = null;
                            pCalendar.GetFiscalYear(m_pdItem.StartDate.AddDays(1), out objFY);
                        }
                    }
                    else if (pdi.PeriodStartDate == beginPeriodEnd.AddDays(1))
                    {
                        pdi.AdjustmentAmt = adjustmentAmt;
                        pdi.FiscalYearBeginAccum = dBeginYearAccum;
                        pdi.FiscalYearDeprAmount = pdi.FiscalYearEndAccum - pdi.FiscalYearBeginAccum;
                        pdi.PeriodBeginAccum = prioAccum;
                        pdi.PeriodEndAccum = pdi.PeriodBeginAccum + pdi.PeriodDeprAmount + pdi.AdjustmentAmt;
                        prioAccum = pdi.PeriodEndAccum;
                    }
                    else
                    {
                        ...
                        pdi.FiscalYearDeprAmount = pdi.FiscalYearEndAccum - pdi.FiscalYearBeginAccum;
                        pdi.PeriodBeginAccum = prioAccum;
                        pdi.PeriodEndAccum = pdi.PeriodBeginAccum + pdi.PeriodDeprAmount;
                        prioAccum = pdi.PeriodEndAccum;
                    }
                }
                else
            ...
            objFY = null;
            pCalendar.GetFiscalYear(m_pdItem.StartDate.AddDays(1), out objFY);
            objFY.GetPeriod(m_pdItem.StartDate.AddDays(1), out objPeriod);
            if (pInList.Count == 0 || m_pdItem.StartDate.AddDays(1) > m_deemedEndDate)//Asset is fully depreciated
            {
                pInList = new List<IBAPeriodDetailDeprInfo>();
                PeriodDetailDeprInfo beginAccumDepr = null;
                //calculating ajustment
                decimal adjAmt = 0;
                //IBADeprMethod pVal =null;
                //IBAAvgConvention pObjAvgConvetion=null;
                //string avgConvention = "";
                //CreateDeprMethod(pObjSch.DeprMethod, false, out pVal);                

                // if(pVal !=null)
                //{                    
                //pVal.GetAvgConvention(pObjSch, ref avgConvention);
                //CreateAvgConvention(avgConvention, out pObjAvgConvetion);
                //InitializeDeprMethod(pObjSch, pObjAvgConvetion, pVal);
                //adjAmt = Convert.ToDecimal(pVal.Basis -  m_pdItem.BeginPeriodAccum);
                double dBUPct = 0;
                double dIUPct = 0;
                double factor = 0;
                pObjSch.GetBusinessUse(beginPeriodEnd, ref dBUPct, ref dIUPct);
                dBUPct += dIUPct;
                factor = CalcUsageFraction(pObjSch, pObjSch.AcquisitionValue, beginPeriodEnd);
                var amt = pObjSch.AcquisitionValue * factor;
                adjAmt = Convert.ToDecimal(amt - (pObjSch.Section179 + m_pdItem.BeginPeriodAccum));
                if (objFY.YRStartDate != objPeriod.PeriodStart) //the begin date doesn't lie in the first period of the fiscal year that means the asset can have begin ytd ammount
                {
                    IBAFiscalYear begYTDobjFY = null;
                    IBACalcPeriod begYTDPeriod;
                    pCalendar.GetFiscalYear(m_pdItem.StartDate, out begYTDobjFY);
                    objFY.GetPeriod(m_pdItem.StartDate, out begYTDPeriod);
                    PeriodDetailDeprInfo p = new PeriodDetailDeprInfo()
                    {
                        FiscalYearBeginAccum = Convert.ToDecimal(m_pdItem.BeginYearAccum),
                        PeriodDeprAmount = Convert.ToDecimal(m_pdItem.BeginYTDExpense),
                        PeriodEndAccum = Convert.ToDecimal(m_pdItem.BeginYTDExpense),
                        PeriodStartDate = begYTDPeriod.PeriodStart,
                        PeriodEndDate = begYTDPeriod.PeriodEnd,
                        FiscalYearStartDate = begYTDobjFY.YRStartDate,
                        FiscalYearEndDate = begYTDobjFY.YREndDate
                    };
                    pInList.Add(p); //add begin ytd info
                    beginAccumDepr = new PeriodDetailDeprInfo()
                    {
                        FiscalYearBeginAccum = p.FiscalYearBeginAccum,
                        PeriodBeginAccum = p.PeriodEndAccum,
                        PeriodDeprAmount = 0,
                        PeriodEndAccum = p.PeriodEndAccum + adjAmt,
                        AdjustmentAmt = adjAmt,
                        PeriodStartDate = objPeriod.PeriodStart,
                        PeriodEndDate = objPeriod.PeriodEnd,
                        FiscalYearStartDate = objFY.YRStartDate,
                        FiscalYearEndDate = objFY.YREndDate
                    };
                }
                if (beginAccumDepr == null)
                {
                    beginAccumDepr = new PeriodDetailDeprInfo()
                    {
                        FiscalYearBeginAccum = Convert.ToDecimal(m_pdItem.BeginYearAccum),
                ...
                pInList.Add(beginAccumDepr); // add the begin accum info
            }
        }

        private void setPeriodDetailDeprInfoForBeginInfoRV(IBADeprScheduleItem pObjSch, IBACalendar pCalendar, ref List<IBAPeriodDetailDeprInfo> pInList)
        {
            IBAFiscalYear objFY;
            IBACalcPeriod objPeriod;
            DateTime beginPeriodStart;
            ...
            decimal dBeginYearAccum = m_pdItem.BeginYearAccum;
            IBAPeriodDetailDeprInfo newPdi = new PeriodDetailDeprInfo();
            bool isnewPdi = false;
            ...
            beginPeriodStart = objPeriod.PeriodStart;
            beginPeriodEnd = objPeriod.PeriodEnd;
            IBAPeriodDetailDeprInfo prevpdi = null;
            foreach (IBAPeriodDetailDeprInfo pdi in pInList)
            {
                if (objFY.YRStartDate <= pdi.PeriodStartDate && pdi.PeriodEndDate <= objFY.YREndDate)
                {
                    pdi.FiscalYearStartDate = objFY.YRStartDate;
                    pdi.FiscalYearDeprAmount = pdi.FiscalYearEndAccum - pdi.FiscalYearBeginAccum;
                    if (pdi.PeriodStartDate == beginPeriodEnd.AddDays(1) && pdi.PeriodStartDate != objFY.YRStartDate)
                    {
                        isnewPdi = true;

                        //adding begin info
                        newPdi.FiscalYearStartDate = pdi.FiscalYearStartDate;
                        newPdi.FiscalYearEndDate = pdi.FiscalYearEndDate;
                        newPdi.FiscalYearBeginAccum = pdi.FiscalYearBeginAccum + Convert.ToDecimal(m_pdItem.BeginYearAccum);
                        newPdi.FiscalYearDeprAmount = 0;
                        newPdi.FiscalYearEndAccum = pdi.FiscalYearEndAccum;
                        newPdi.PeriodBeginAccum = 0;
                        newPdi.PeriodDeprAmount = m_pdItem.BeginYTDExpense;
                        newPdi.PeriodEndAccum = m_pdItem.BeginYTDExpense;
                        newPdi.PeriodStartDate = beginPeriodStart;
                        newPdi.PeriodEndDate = beginPeriodEnd;

                        pdi.FiscalYearBeginAccum = pdi.FiscalYearBeginAccum + Convert.ToDecimal(m_pdItem.BeginYearAccum);
                        pdi.PeriodBeginAccum = m_pdItem.BeginYTDExpense;
                        pdi.PeriodEndAccum = pdi.PeriodBeginAccum + pdi.PeriodDeprAmount + pdi.AdjustmentAmt;

                    }
                    else if (isnewPdi && prevpdi != null)
                    {
                        pdi.FiscalYearBeginAccum = pdi.FiscalYearBeginAccum + Convert.ToDecimal(m_pdItem.BeginYearAccum);
                        pdi.PeriodBeginAccum = prevpdi.PeriodEndAccum;
                        pdi.PeriodEndAccum = pdi.PeriodBeginAccum + pdi.PeriodDeprAmount + pdi.AdjustmentAmt;
                    }
                    prevpdi = pdi;
                }
            ...
            if (isnewPdi)
            {
                pInList.Insert(0, newPdi);
            }
        }


        public void SetAutoTruckLimit(string autoTruckLimit)
        {
            int i = 0;
            if (autoTruckLimit == null || autoTruckLimit == string.Empty)
                return;
            string[] sLimit = autoTruckLimit.Split(',');
            foreach (string tmp in sLimit)
            {
                m_dLimitArray[i++] = Convert.ToDouble(tmp);
            }
        }
    }

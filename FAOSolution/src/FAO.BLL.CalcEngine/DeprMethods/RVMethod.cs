...
namespace FAO.BLL.CalcEngine
{
    class RVMethod : IBADeprMethod
    {
        ...
        {
            get { return "Remaining Value/Remaining Life"; }
        }
        ...
        public double CalculateAnnualDepr()
        {
            ...
            pVal = (dBasis - m_dPriorAccum) / (m_dLife - m_dYearElapsed);
            if (!m_PisFys && m_dLife < 1 && (int)m_dDBPercent < 100)
                return pVal;

            if (Math.Abs(pVal) > dBasis && m_dLife < 1 && (int)m_dDBPercent < 100)
            {
                pVal = dBasis;
            }
            return pVal;

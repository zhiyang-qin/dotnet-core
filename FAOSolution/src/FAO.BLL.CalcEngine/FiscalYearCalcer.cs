    ...
    public class FiscalYearCalcer
    {
                                        ...
                                        IBAPeriodDeprItem pObjStartPoint)
        {
            ...
            if (bRet)
            {
                ...
                dDeprAmt = Currency.FormatCurrency(CurrencyHelper.FormatCurrency(dFraction * dActualDeprAmt));
                                                          ...
                                                          out IBAPeriodDeprItem pditem2)
        {
            ...
            if (!(hr = m_pObjIFY.GetPeriod(dtStart, out pObjPeriod)))
                return false;
            //Kent: if there is only one period in the newItem then don't need to split this item
            if (dtEnd == pObjPeriod.PeriodEnd)
                return true;

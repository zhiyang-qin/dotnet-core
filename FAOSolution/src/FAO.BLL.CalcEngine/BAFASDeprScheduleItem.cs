...
namespace FAO.BLL.CalcEngine
{
    ...
    public class BAFASDeprScheduleItem : IBADeprScheduleItem, IBADeprTable
    {
        ...
        double m_ClassLife;
        AssetImportCodeEnum m_ImportCode;
        AssetITCTypeEnum m_ITCCode;
        AssetDeprAdjustmentConventionEnum m_ApplyAdjustment;
        BkTypeEnum m_bookType;
        double m_AcquisitionValue;
        double m_ITCAmount;
        double m_ITCReduce;
        AssetCreationCodeEnum m_creationCode;
        DateTime m_TransferInDate;
        DateTime m_EffectiveTransferInDate;
        DateTime m_TransferOutDate;
        AssetActivityTypeEnum m_activityCode;
        bool m_reduceBasisByITC = true;
        double[] m_Pcts = new double[MAXPOINTS];
        short m_PctsUsed = 0;
        ...
        public bool UseACEHandling
        {
            get
            {
                ...
                if (m_bookType == BkTypeEnum.bpblBookACEBook)
                {
                    //        *pVal = VARIANT_TRUE;
        ...
        public DateTime DispDate
        {
            get
            {
                if (m_activityCode == AssetActivityTypeEnum.acWholeTransferDisposed ||
                    m_activityCode == AssetActivityTypeEnum.acPartialTransferDisposed)
        ...
        public AssetImportCodeEnum ImportCode
        {
            get { return m_ImportCode; }
            set { m_ImportCode = value; }
        }

        public AssetITCTypeEnum ITCCode
        {
            get { return m_ITCCode; }
            set { m_ITCCode = value; }
        }

        public AssetDeprAdjustmentConventionEnum ApplyAdjustment
        {
        ...
        public bool PersonalPropertyFlag
        {
            get
            {
                return ((m_PropertyType == 0 || m_PropertyType == 1 || m_PropertyType == 8 || m_PropertyType == 13) ? true : false);
            }
        }
        ...
        public string AutoTruckLimit
        {
            get { return m_AutoTruckLimit; }
            set { m_AutoTruckLimit = value; }
        }
        ...
        AssetCreationCodeEnum CreationCode
        {
        ...
        AssetActivityTypeEnum ActivityCode
        {
        ...
        public double CalculateBonus911Amount
        {
            get
            {
                ...
                {
                    if (m_dtPlacedInServiceDate > new DateTime(2014, 12, 31) && m_luxuryLimit == LUXURYLIMIT_TYPE.LUXURYLIMITTYPE_LUXURYCAR)
                        dAutoLimit = 3160.0;
                    else if (m_dtPlacedInServiceDate > new DateTime(2011, 12, 31) && m_luxuryLimit == LUXURYLIMIT_TYPE.LUXURYLIMITTYPE_LUXURYCAR)
                        dAutoLimit = 3160.0;
                    ...
                    {
                        if (m_dtPlacedInServiceDate > new DateTime(2014, 12, 31))
                            dAutoLimit = 3460.0;
                        else if (m_dtPlacedInServiceDate > new DateTime(2013, 12, 31))
                            dAutoLimit = 3460.0;
                        else if (m_dtPlacedInServiceDate > new DateTime(2011, 12, 31))
                            dAutoLimit = 3360.0;
                        else if (m_dtPlacedInServiceDate > new DateTime(2010, 12, 31))
                            dAutoLimit = 3260.0;
                        else if (m_dtPlacedInServiceDate > new DateTime(2009, 12, 31))
                            dAutoLimit = 3160.0;
                            ...
                            dAutoLimit = 3260.0;
                        else if (m_dtPlacedInServiceDate > new DateTime(2002, 12, 31))
                            dAutoLimit = 3360.0;
                        else if (new DateTime(2001, 9, 11) <= m_dtPlacedInServiceDate && m_dtPlacedInServiceDate <= new DateTime(2012, 12, 31))
                        {
                        }
                        else
                            dAutoLimit += 300;
                ...
                if (dAutoLimit > 0.0)
                {
                    ...
                    else if (new DateTime(2008, 1, 1) <= m_dtPlacedInServiceDate && m_dtPlacedInServiceDate <= new DateTime(2014, 12, 31))
                    {
                ...
                if (count > 0)
                {
                    ...
                    GetBusinessUse(m_dtPlacedInServiceDate, ref busUse, ref investUse);
        ...
        public double CalculateITCBasisReductionAmount(bool AceSwitch)
        {
            ...
            if (AceSwitch == false && m_ImportCode == AssetImportCodeEnum.icPCFasImport)
            {
        ...
        public iTCCodes _convertITCStatus(AssetITCTypeEnum itcCode)
        {
            switch (itcCode)
            {
                case AssetITCTypeEnum.itcNoITC:
                    return iTCCodes.NO_ITC;
                case AssetITCTypeEnum.itcNewPropFullCredit:
                    return iTCCodes.NEW_PROP_FULL;
                case AssetITCTypeEnum.itcNewPropReducedCredit:
                    return iTCCodes.NEW_PROP_RED;
                case AssetITCTypeEnum.itcUsedPropFullCredit:
                    return iTCCodes.USED_PROP_FULL;
                case AssetITCTypeEnum.itcUsedPropReducedCredit:
                    return iTCCodes.USED_PROP_RED;
                case AssetITCTypeEnum.itcRehab30Year:
                    return iTCCodes.THIRTY_YR_RE;
                case AssetITCTypeEnum.itcRehab40Year:
                    return iTCCodes.FORTY_YR_RE;
                case AssetITCTypeEnum.itcCertHistoricRehab:
                    return iTCCodes.CERT_HISTORIC;
                case AssetITCTypeEnum.itcNonCertHistoricRehab:
                    return iTCCodes.NON_HISTORIC;
                case AssetITCTypeEnum.itcBiomass:
                    return iTCCodes.BIOMASS_PROP;
                case AssetITCTypeEnum.itcIntercityBuses:
                    return iTCCodes.INTER_CITY_BUS;
                case AssetITCTypeEnum.itcHydroelectricGenerating:
                    return iTCCodes.HYDRO_GEN_PLANT;
                case AssetITCTypeEnum.itcOceanThermal:
                    return iTCCodes.OCEAN_THRM_PROP;
                case AssetITCTypeEnum.itcSolarEnergy:
                    return iTCCodes.SOLAR_ENERGY;
                case AssetITCTypeEnum.itcWind:
                    return iTCCodes.WIND_PROP;
                case AssetITCTypeEnum.itcGeoThermal:
                    return iTCCodes.GEO_THRM_PROP;
                case AssetITCTypeEnum.itcCertHistoricTransition:
                    return iTCCodes.ITCP;
                case AssetITCTypeEnum.itcQualifiedProgressExp:
                    return iTCCodes.ITCQ;

                case AssetITCTypeEnum.itcSolarEnergyProperty:
                    return iTCCodes.ITCS;
                case AssetITCTypeEnum.itcOtherEnergyProperty:
                    return iTCCodes.ITCT;
                case AssetITCTypeEnum.itcFuelCellProperty:
                    return iTCCodes.ITCU;
                case AssetITCTypeEnum.itcMicroturbineProperty:
                    return iTCCodes.ITCV;
                case AssetITCTypeEnum.itcAdvancedCoalProject:
                    return iTCCodes.ITCW;
                case AssetITCTypeEnum.itcGasificationProject:
        ...
        public void CopyPercentsToArray(ref double[] retVal)
        {
            for (int i = 0; i < m_Pcts.Length; i++)
                retVal[i] = m_Pcts[i];
        }

        public void CopyPercentsFromArray(double[] inVal)
        {
            m_PctsUsed = (short)inVal.Length;
            for (int i = 0; i < inVal.Length; i++)
                m_Pcts[i] = inVal[i];

        }

        public IBADeprTable GetCustomDepreciationTable
        {
            get
            {
                IBADeprTable table = (this) as IBADeprTable;
                return table;
            }
        }
        ...
        public short YearCount
        {
            ...
            {
                short count;
                for (count = m_PctsUsed; count > 0 && Math.Abs(m_Pcts[count - 1]) < 0.0000001; count--)
                {
                }
                return count;
            }
        ...
        {
            pct = 1;
            if (year < 1 || year > m_PctsUsed || period != 1)
                return false;
